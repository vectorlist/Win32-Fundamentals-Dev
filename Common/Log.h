...
LPCSTR LOG::WndMessage(UINT msg)
{
	static std::map<UINT, LPCSTR> codes = {
{0, "WM_NULL" },
{1, "WM_CREATE" },
{2, "WM_DESTROY" },
{3, "WM_MOVE" },
...
inline void LOG::LogWndMessage(UINT msg, UINT filterMsg)
{
	printf("[%s]\n", WndMessage(msg));
}

inline void LOG::LogCtrlCustomDrawStage(LPARAM lp)
{
...
inline void LOG::LogLastError() {
	...
	FormatMessage(
		...
		MAKELANGID(LANG_ENGLISH, SUBLANG_DEFAULT), (LPTSTR)&lpBuf, 0, NULL);
	//code.clear();

	std::string code((LPCSTR)lpBuf);
	code.append(__FILE__);
